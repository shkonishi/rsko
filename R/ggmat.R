...
#' @usage ggmat(dat, column, xfctr, lwd, mcols, col_fctr, vselect, vcol, xlab, ylab, main)
  ...
  if (!is.null(col_fctr) & length(col_fctr) == nrow(dat)) {
    ...
    if (is.null(mcols)) {
      res <- data.frame(xfctr, col_fctr, dat) %>%
        ...
        ggplot2::ggplot(., ggplot2::aes(x = xfctr, y = value,
                                        colour = col_fctr, group = interaction(key, col_fctr))) +
      ...
      res <- data.frame(xfctr, col_fctr, dat) %>%
        ...
        ggplot2::ggplot(., ggplot2::aes(x = xfctr, y = value,
                                        colour = col_fctr, group = interaction(key, col_fctr))) +
        ggplot2::geom_line(size = lwd) +
        ggplot2::scale_color_manual(values = mcols) +
        ggplot2::theme_minimal(base_size = 20) +
        ggplot2::labs(title = main, x = xlab, y = ylab, colour = "")
    ...
    if (is.null(vselect)) {# no coloured
      ...
      if (is.null(names(vselect))) { # High lighted lines with selected variable
        ...
        ## High lighted lines with names of selected variable if 'vselect' was named vector, names of 'vselect' were
        ## factor for colours. ----
        gdat <- data.frame(xfctr, dat) %>%
          tidyr::gather(., key = "key", value = "value", -1) %>%
